...
pub const Bson = struct {
    value: c.bson_t, // pointer to C BSON struct

    const Self = @This();

    fn get(self: *Self) *c.bson_t {
        return &self.value;
    }

    fn getConst(self: *const Self) *const c.bson_t {
        return &self.value;
    }
    ...
    pub fn arrayAsJson(self: *Self) !Json {
        ...
        if (c.bson_array_as_json(self.get(), &l)) |value| {
            return Json{
                .value = value,
            };
        } else {
    ...
    pub fn appenArray(self: *Self, key: [:0]const u8, array: *Bson) !void {
        const ok = c.bson_append_array(self.get(), key, -1, array.get());
        if (!ok) {
            return Error.BsonError;
        }
    ...
    pub fn appendArrayBegin(self: *Self, key: [:0]const u8, child: *Bson) !void {
        const ok = c.bson_append_array_begin(self.get(), key, -1, child.get());
        if (!ok) {
            return Error.BsonError;
        }
    ...
    pub fn appendArrayEnd(self: *Self, child: Bson) !void {
        const ok = c.bson_append_array_end(self.get(), child.get());
        if (!ok) {
            return Error.BsonError;
        }
    ...
    pub fn appendBinary(self: *Self, key: [:0]const u8, binary: [:0]u8) !void {
        const ok = c.bson_append_array_begin(self.get(), key, -1, binary, -1);
        if (!ok) {
            return Error.BsonError;
        }
